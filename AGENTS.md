...
komo skill list                   # governed skill store: active skills + reviewer candidates
komo skill install <source>       # fetch a skill (owner/repo[/subpath], GitHub/*.git/git@ URL, or a raw SKILL.md URL) straight into the active store
komo skill inspect <name>         # one skill in full: status, provenance, path, history, body
komo skill promote|reject <name>  # triage a reviewer candidate (accept into active / discard)
komo skill protect|unprotect <name>  # operator-edit-only: reviewer stops proposing changes
komo skill enable|disable <name>  # hide from the agent without deleting (and back)
komo skill audit <name>           # which turns loaded this skill (derived from the run ledger)
